...
import co.razkevich.sflocalstack.service.MetadataManifestParser;
import co.razkevich.sflocalstack.service.MetadataService;
import co.razkevich.sflocalstack.service.MetadataSoapParser;
import co.razkevich.sflocalstack.service.MetadataSoapRenderer;
    ...
    public ResponseEntity<String> handle(@RequestBody String body) {
        try {
            ...
            String response = switch (request.operation()) {
                ...
                case "cancelDeploy" -> {
                    Object cancelId = request.values().containsKey("asyncProcessId")
                            ? request.values().get("asyncProcessId")
                            : request.values().get("String");
                    yield renderer.renderCancelDeploy(metadataService.cancelDeploy(String.valueOf(cancelId)));
                }
